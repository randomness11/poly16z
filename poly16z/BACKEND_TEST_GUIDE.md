...
Add to your `.env` file (or `poly16z/.env`):
...
DATABASE_URL=sqlite+aiosqlite:///poly16z.db
...
```bash
cd ~/polymarket-ai-bot/poly16z
python main.py --strategy custom --agent openai --dry-run
```

You should see:
```
🚀 Starting Poly16z Bot [Strategy: custom] [Agent: openai]
...
# Expected: {"message":"poly16z Dashboard API","version":"1.0.0"}
```
...
6. ✅ Database file `poly16z.db` is created
